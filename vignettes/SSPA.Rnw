...
effectsize <- data.frame(exposure = factor(rep(rep(c("5 Days", "6 Hours"), c(2, 3)), each=1024)), 
                         ...
                         lambda = as.vector(sapply(ss, function(x)x@lambda)), 
                         theta = as.vector(sapply(ss, function(x)x@theta)))
...
samplesize <- seq(2, 8)
averagepower <- data.frame(power = as.vector(sapply(ss, function(x) as.numeric(predictpower(x, samplesize=sqrt(samplesize))))), 
